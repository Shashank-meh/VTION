...
**/
FMRadioApp.factory('UserService', function($http, $q, $Toaster) {
	...
	function sendUserOtp(userjOb){
		...
		var deferred = $q.defer();
		if(deviceInfo.checkConnection()){
			authRemoteService.resendOtp(userInfo.getUserId(), parseInt(userjOb.mno), userjOb.em).then(function(response) {
				if (response.data.msg == "success") {
					//$Toaster.showSuccess(response.data.msg);
					deferred.resolve(true);
				} else {
